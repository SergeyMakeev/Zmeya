...
TEST(ZmeyaTestSuite, NewBuilderAPI_BasicTypes)
{
    ...
    // Test HashMap
    std::unordered_map<std::string, int32_t> srcMap = {
        {"one", 1}, {"two", 2}, {"three", 3}
    };
...
TEST(ZmeyaTestSuite, NewBuilderAPI_NestedTypes)
{
    ...
    std::vector<std::vector<std::string>> srcNested = {
        {"a", "b", "c"},
        {"x", "y"},
        {"hello", "world", "nested", "test"}
    };
...
TEST(ZmeyaTestSuite, NewBuilderAPI_EmptyContainers)
{
    // Create builder
    auto builder = zm::Builder::create();
    ...
    std::vector<int32_t> emptyIntArray;
    std::unordered_map<std::string, int32_t> emptyMap;
    std::unordered_set<std::string> emptySet;
    
    zm::assign(root->stringArray, emptyStringArray);
    zm::assign(root->intArray, emptyIntArray);
    zm::assign(root->hashMap, emptyMap);
    zm::assign(root->hashSet, emptySet);
    ...
    EXPECT_TRUE(fileRoot->stringArray.empty());
    EXPECT_TRUE(fileRoot->intArray.empty());
    EXPECT_TRUE(fileRoot->hashMap.empty());
    EXPECT_TRUE(fileRoot->hashSet.empty());
}
