...
- **Incremental policy:** `zm_hashset_chain_incremental_ok<Key>` and `zm_hashmap_chain_incremental_ok<Key, Value>` are **`std::true_type`** for API documentation; incremental **`hashset_*` / `hashmap_*`** always use the chain builders (**`hashmap_chain_*` / `hashset_chain_*`**) with **`hash_chain_nodes_array_grow_append_default_*`** in **`ZmeyaBuilderHashChainNodesGrow.inc`** for **`nodes[]`** slab growth (no per-op full-table **`std::unordered_*`** snapshot).
